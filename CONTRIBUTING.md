...
Thank you for considering making contributions to Mage and related
repositories!
...
1. Either [open](https://github.com/warmage-sports/mage/issues/new/choose) or
   [find](https://github.com/warmage-sports/mage/issues) an issue you'd like to help with
...
3. If you would like to contribute:
    ...
    4. Follow standard GitHub best practices: fork the repo, branch from the
       HEAD of `master`, make some commits, and submit a PR to `master`
        - For core developers working within the Mage repo, to ensure a clear
...
- Looking for a good place to start contributing? How about checking out some
  [good first issues](https://github.com/warmage-sports/mage/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22)
...
When proposing an architecture decision for Mage, please start by opening an [issue](https://github.com/warmage-sports/mage/issues/new/choose) or a [discussion](https://github.com/warmage-sports/mage/discussions/new) with a summary of the proposal. Once the proposal has been discussed and there is rough alignment on a high-level approach to the design, the [ADR creation process](https://github.com/warmage-sports/mage/blob/master/docs/architecture/PROCESS.md) can begin. We are following this process to ensure all involved parties are in agreement before any party begins coding the proposed implementation. If you would like to see examples of how these are written, please refer to the current [ADRs](https://github.com/warmage-sports/mage/tree/master/docs/architecture) or to [Cosmos ADRs](https://github.com/cosmos/cosmos-sdk/tree/master/docs/architecture).
...
If you open a PR on Mage, it is mandatory to update the relevant documentation inside the  [docs repo](https://github.com/desmos-labs/docs).
...
While my fork lives at `https://github.com/riccardom/mage`,
the code should never exist at `$GOPATH/src/github.com/riccardom/mage`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,
`$GOPATH/src/github.com/warmage-sports/mage`, and do all the work there.

For instance, to create a fork and work on a branch of it, I would:

- Create the fork on GitHub, using the fork button.
- Go to the original repo checked out locally (i.e. `$GOPATH/src/github.com/warmage-sports/mage`)
- `git remote rename origin upstream`
- `git remote add origin git@github.com:riccardom/mage.git`

Now `origin` refers to my fork and `upstream` refers to the Mage version.
So I can `git push -u origin master` to update my fork, and make pull requests to Mage from there.
...
The master branch of every Mage repository should just build with `go get`,
...
We use [Protocol Buffers](https://developers.google.com/protocol-buffers) along with [gogoproto](https://github.com/gogo/protobuf) to generate code for use in Mage.
...
Tests can be run by running `make test` at the top level of the Mage repository.
...
Mage utilizes [semantic versioning](https://semver.org/).
...
- create a development branch either on github.com/warmage-sports/mage, or your fork (using `git remote add origin`)
...
The process for how Mage maintainers take features and ADRs from concept to release
is broken up into three distinct stages: **Strategy Discovery**, **Concept Approval**, and
**Implementation & Release Approval**

### Strategy Discovery

* Develop long term priorities, strategy and roadmap for Mage
* Release committee not yet defined as there is already a roadmap that can be used for the time being

### Concept Approval

* Architecture Decision Records (ADRs) may be proposed by any contributors or maintainers of Mage,
  and should follow the guidelines outlined in the
  [ADR Creation Process](https://github.com/warmage-sports/mage/blob/master/docs/architecture/PROCESS.md)
