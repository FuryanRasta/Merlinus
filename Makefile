...
HTTPS_GIT := https://github.com/warmage-sports/mage.git
...
build-alpine: create-builder
	mkdir -p $(BUILDDIR)
	$(DOCKER) build -f Dockerfile --rm --tag magelabs/mage-alpine .
	$(DOCKER) create --name mage-alpine --rm magelabs/mage-alpine
	$(DOCKER) cp mage-alpine:/usr/bin/mage $(BUILDDIR)/mage
	$(DOCKER) rm mage-alpine

build-linux: create-builder
	mkdir -p $(BUILDDIR)
	$(DOCKER) build -f Dockerfile-ubuntu --rm --tag magelabs/mage-linux .
	$(DOCKER) create --name mage-linux magelabs/mage-linux
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' -not -path "./venv" | xargs goimports -w -local github.com/warmage-sports/mage
.PHONY: format
...
localnet-start: localnet-stop setup-localnet
	$(if $(shell docker inspect -f '{{ .Id }}' magelabs/mage-env 2>/dev/null),$(info found image magelabs/mage-env),$(MAKE) -C contrib/images mage-env)
	docker-compose up -d
