...
func GetCmdLinkChainAccount() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Link a Mage profile to an external chain account using the data written inside the given file",
		Long: strings.TrimSpace(fmt.Sprintf(`Link an external account to a Mage profile.
...
%s query profiles chain-links mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud
%s query profiles chain-links mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud "cosmos"
%s query profiles chain-links mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud "cosmos" cosmos19s242dxhxgzlsdmfjjg38jgfwhxca7569g84sw
...
%s query profiles default-external-address "mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud"
%s query profiles default-external-address "mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud" "cosmos"
`, version.AppName, version.AppName, version.AppName, version.AppName),
		Args: cobra.RangeArgs(0, 2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
