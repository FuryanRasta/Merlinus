...
import (
	...
	subspacesauthz "github.com/mage-war/mage/x/subspaces/authz"
...
The name must be a human readable name.`,
		Example: fmt.Sprintf(`
%s tx subspaces create "Mage" \
  --description "The official subspace of Mage" \
  --treasury mage1jqk5p244yl4ktukq5xhavvlfzl8z4we4qfmuyh \
  --owner mage1p8r4guvdze03md4g9zclhh6mr8ljvtd80pehr3 \
  --from alice
`, version.AppName),
...
%s tx subspaces edit 1 \
  --name "Mage - Democratizing social networks"
  --description "The official subspace of Mage" \
  --treasury mage1jqk5p244yl4ktukq5xhavvlfzl8z4we4qfmuyh \
  --owner mage1p8r4guvdze03md4g9zclhh6mr8ljvtd80pehr3 \
  --from alice
...
func GetCmdAddUserToUserGroup() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`
%s tx subspaces groups add-user 1 1 mage1p8r4guvdze03md4g9zclhh6mr8ljvtd80pehr3 \
  --from alice
`, version.AppName),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
...
func GetCmdRemoveUserFromUserGroup() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`
%s tx subspaces groups remove-user 1 1 mage1p8r4guvdze03md4g9zclhh6mr8ljvtd80pehr3 \
  --from alice
`, version.AppName),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
...
%s tx subspaces set-user-permissions 1 \
  mage1463vltcqk6ql6zpk0g6s595jjcrzk4804hyqw7 \
...
If you want to grant the same authorization inside multiple subspaces, simply specify the subspaces ids separating them with a comma (,).`,
		Example: fmt.Sprintf(`
%s tx subspaces grant 1,2,3 mage1463vltcqk6ql6zpk0g6s595jjcrzk4804hyqw7 --msg-type=%s --from alice
