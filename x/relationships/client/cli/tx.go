...
func GetCmdCreateRelationship() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s tx relationships create-relationship mage1e209r8nc8qdkmqujahwrq4xrlxhk3fs9k7yzmw 1`, version.AppName),
...
func GetCmdDeleteRelationship() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s tx relationships delete-relationship mage1e209r8nc8qdkmqujahwrq4xrlxhk3fs9k7yzmw 1`, version.AppName),
		Args:    cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func GetCmdBlockUser() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s tx relationships block mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud 1
%s tx relationships block mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud 1 "reason for block"`, version.AppName, version.AppName),
...
func GetCmdUnblockUser() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s tx relationships unblock mage13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud 1`, version.AppName),
		Args:    cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
