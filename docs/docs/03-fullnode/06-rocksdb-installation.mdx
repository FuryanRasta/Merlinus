...
# Installing RocksDB
By default, Mage uses [LevelDB](https://github.com/google/leveldb) as its database backend engine. However, since version 
...
so that you will be able to use as Mage' backend engine for better performances.
...
# Install RocksDB so that Mage can access it
...
Once the installation has finished, you will be able to compile Mage using the following command: 
