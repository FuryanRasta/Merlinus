...
Inside Mage, there are various types of pruning strategies that can be applied. The main ones are:
...
In order to run a fullnode, you need to build `mage` which requires `Go`, `git`, `gcc` and `make` installed.
...
Once you have set up your environment correctly, you are now ready to install the Mage software and start your full
node.
