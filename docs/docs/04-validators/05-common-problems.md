...
If you got jailed for downtime, you can get your voting power back to your validator. First, if `mage` is not running,
start it up again:
...
## Problem #2: My `mage` crashes because of `too many open files`

The default number of files Linux can open (per-process) is `1024`. `mage` is known to open more than `1024` files.
This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed) and
then restart the process with `mage start`. If you are using `systemd` or another process manager to launch `mage`
...
ExecStart=/home/ubuntu/go/bin/mage start # The path should point to the correct location of the software you have installed.
Restart=on-failure
RestartSec=3
...
1. Get your address:
   ```bash
   mage keys show <your_key> --address
   ```
...
3. Make sure the tokens have been sent properly:
   ```bash
   mage query account $(mage keys show <your_key> --address) --chain-id <chain_id>
   ```
   
4. Delegate the tokens to your validator: 
   ```bash
   mage tx staking delegate \
     $(mage keys show <your_key> --bech=val --address) \
     <amount> \
     --chain-id <chain_id> \
     --from <your_key> --yes
   ...
   # Example
   # mage tx staking delegate \
   #  $(mage keys show validator --bech=val --address) \
   #  10000000udaric \
...
number of blocks. We suggest you checking the Mage daemon status to make sure it hasn't been interrupted by some
error.
...
```bash
mage tx slashing unjail --chain-id <chain_id> --from <your_key>

# Example
# mage tx slashing unjail --chain-id morpheus-apollo-2 --from validator
```
...
If your service is running properly, you can also try and reset your `mage` configuration by running the following
command:

```bash
rm $HOME/.mage/config/config.toml
``` 

After doing so, remember to restart your validator service to apply the changes:

```bash
systemctl restart maged
```
...
3. Replace that line with the following: 
   ```
   seeds = "cd4612957461881d5f62367c589aaa0fdf933bd8@seed-1.morpheus.mage.network:26656,fc4714d15629e3b016847c45d5648230a30a50f1@seed-2.morpheus.mage.network:26656"
   ```
...
For example, you can use the public RPC endpoint [here](https://rpc.morpheus.mage.network/net_info). 
...
the `persistent_peers` field of your `~/.mage/config/config.toml` file.
...
The space you need is directly related to the pruning strategy your using, you can read more about it [here](https://docs.mage.network/fullnode/overview#understanding-pruning).
...
1) Stop your node daemon service (usually sudo `systemctl stop maged`);
2) Navigate to `.mage/config/` and open app.toml;
...
5) Navigate to .mage/data/ and backup the `priv_validator_state.json` file (this will keep the voting state avoiding double sign); 
6) Unsafe reset your node with `mage unsafe-reset-all`;
7) Place the backup of `addrbook.json` and `priv_validator_state.json` back into `.mage/config/` and `.mage/data/` folders respectively;
...
If you run into this problem while you're syncing your node, probably you are using a wrong Mage binary version.
