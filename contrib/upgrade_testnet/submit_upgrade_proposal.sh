...
TX_FLAGS="--from node0 --keyring-backend test --home ./build/node0/mage --chain-id $CHAIN_ID --yes --fees 100udaric"
...
for ((i = 1; i < $NODES; i++)); do
  echo "====> Node $i"
  NODE_SECRET=$(cat "build/node$i/mage/key_seed.json" | jq .secret -r)
  echo $NODE_SECRET | mage keys add "node$i" --recover --home ./build/node0/mage --keyring-backend test >/dev/null 2>&1
done
...
RESULT=$(mage tx gov submit-proposal \
...
PROPOSAL_ID=$(mage q tx $TX_HASH --node $NODE --output json 2>&1 | jq .logs[0].events[4].attributes[0].value -r)
echo "Proposal ID: $PROPOSAL_ID"

echo ""
echo "===> Depositing proposal"
mage tx gov deposit $PROPOSAL_ID 10000000udaric $TX_FLAGS >/dev/null 2>&1
...
for ((i = 0; i < $NODES; i++)); do
  echo "====> Node $i"
  mage tx gov vote $PROPOSAL_ID yes $TX_FLAGS --from "node$i" >/dev/null 2>&1
done
